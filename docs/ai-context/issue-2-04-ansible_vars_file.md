# Issue #2-04: Ansible 변수 정의 및 VM 리스트 구조화 (네트워크/ID 규칙 수정)

## 📅 날짜: 2025-12-29
## 👤 참여자: 임성현, Gemini

## 💬 대화 요약
1. **IP 대역대 조정**: 
   - Ubuntu 기반 신규 VM: `.13` 이후인 `.14`부터 순차 할당.
   - K8s 기반 신규 VM: `.30`부터 순차 할당.
2. **VMID 체계 변경**:
   - K8s 전용 노드들의 VMID를 `300`번부터 시작하도록 변경.
3. **상태 관리 로직**:
   - 모든 신규 VM의 `existing` 기본값을 `false`로 설정.
   - 프로비저닝 완료 후 수동 또는 스크립트로 `true`로 변경하여 중복 생성을 방지하는 구조 채택.

## 🛠️ 결정된 사항
- `vars/vm_list.yml`의 IP 및 ID 체계를 아래 확정안으로 업데이트함.
- `existing: false` 항목만 필터링하여 앤서블 프로비저닝 루프를 수행함.

---

## 📊 최종 확정 VM 리스트 (모니터링 역할 수정 반영)

| 구분 | VM ID | Hostname | IP (192.168.0.x) | Target Node | Template | 비고 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **기존** | 100 | `vm-bastion` | .10 | server-06 | - | Tailscale/Ansible Host |
| **기존** | 101 | `kafka-01` | .11 | server-01 | - | Broker 01 |
| **기존** | 102 | `kafka-02` | .12 | server-02 | - | Broker 02 |
| **기존** | 103 | `kafka-03` | .13 | server-03 | - | Broker 03 |
| **신규** | **104** | `minio-server` | .14 | server-04 | 901 | Object Storage (VM) |
| **신규** | **105** | `postgres-dw` | .15 | server-05 | 901 | Data Warehouse (VM) |
| **신규** | **106** | **`infra-prom-01`** | .16 | **server-06** | 901 | **VM/Host Prometheus 전용** |
| **신규** | **300** | `k8s-master-01` | .30 | server-01 | 902 | Control Plane |
| **신규** | **301** | `k8s-worker-01` | .31 | server-02 | 902 | Spark Worker 01 |
| **신규** | **302** | `k8s-worker-02` | .32 | server-03 | 902 | Spark Worker 02 |
| **신규** | **303** | `k8s-worker-03` | .33 | server-04 | 902 | Airflow/Services |
| **신규** | **304** | `data-gen-worker` | .35 | server-05 | 902 | Spike Generator |
| **신규** | **305** | **`k8s-monitor-01`** | .36 | **server-06** | 902 | **Grafana + K8s Prom Stack** |

---

## 🏗️ 모니터링 아키텍처 흐름

1.  **인프라 레이어**: 6대의 Proxmox 호스트와 Kafka/MinIO/Postgres VM들에 `node-exporter` 설치.
2.  **수집 레이어 (VM)**: `infra-prom-01(106)`이 Proxmox 호스트 및 개별 VM 메트릭 수집.
3.  **수집 레이어 (K8s)**: `k8s-monitor-01(305)`의 Prometheus Operator가 클러스터 내부 메트릭 수집.
4.  **시각화 레이어**: **K8s 내부 Grafana(305)**가 두 프로메테우스 데이터를 통합 쿼리:
    * **Internal**: `http://prometheus-operated.monitoring.svc:9090`
    * **External**: `http://192.168.0.16:9090` (infra-prom-01)

---
**Next Step**: `group_vars/all.yml` 생성 및 `provision_vms.yml` 플레이북 구현.